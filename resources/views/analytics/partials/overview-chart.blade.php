<div class="bg-white rounded-lg border border-gray-200 p-5" x-data="{ tab: 'sessions' }">
    <div class="flex items-center gap-1 mb-4 border-b border-gray-100 -mx-5 px-5 pb-3">
        <button @click="tab = 'sessions'" :class="tab === 'sessions' ? 'text-blue-600 border-b-2 border-blue-600 font-semibold' : 'text-gray-500 hover:text-gray-700'" class="flex items-center gap-1.5 px-3 pb-2 text-sm transition-colors">
            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z"/></svg>
            Sessions
        </button>
        <button @click="tab = 'pageviews'" :class="tab === 'pageviews' ? 'text-blue-600 border-b-2 border-blue-600 font-semibold' : 'text-gray-500 hover:text-gray-700'" class="flex items-center gap-1.5 px-3 pb-2 text-sm transition-colors">
            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z"/><path d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
            Pageviews
        </button>
    </div>
    <div style="height: 280px;">
        <canvas id="sessionsChart" x-show="tab === 'sessions'"></canvas>
        <canvas id="pageviewsChart" x-show="tab === 'pageviews'"></canvas>
    </div>
</div>
